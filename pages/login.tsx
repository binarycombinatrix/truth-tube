...
export default function Login() {
      ...
      if (signInResult.isSignedIn === true) {
        ...
        if (old_user) {
          ...
          if (old_user.username && old_user.dn && old_user.dp) {
            ...
            old_user.subbedto && localStorage.setItem('subbedto', JSON.stringify(old_user.subbedto))
            old_user.likedTo && localStorage.setItem('likedTo', JSON.stringify(old_user.likedTo))
            setProfile({
              dn: old_user.dn,
          ...
          const { errors: err, data: new_user } = await client.models.Video.create(
            {
              partitionKey: email,
              sortKey: email,
              type: 'user_profile',
  ...
  const updateProfileData = async () => {
    try {
      if (email !== '' || localStorage.getItem('email')) {
        const keyval = email !== '' ? email : localStorage.getItem('email') ?? ''
        const newUsername = localStorage.getItem('username') ?? profile.dn + uuidv1()
        ...
        if (errors) {
          ...
          const { data: channelData, errors } = await client.models.Video.create(
            {
      ...
      {showProfile === true ? (
        <div className="upload-form">
          <div className="form-group">
            ...
            <input type="text" id="dn" name="dn" required value={profile.dn} onChange={(e) => setProfile({ ...profile, dn: e.target.value })} />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <input type="file" id="dp" name="dp" onChange={handleDpInput} required />

            <div>
              <button type="button" onClick={uploadDp}>
                Upload
              </button>
              {dpPath && dpPath != '' && <StorageImage className="upload-preview" path={dpPath} alt="Profile Picture" />}
            </div>
          </div>
          <button type="button" className="submit-button" onClick={updateProfileData}>
            Update
          </button>
          {/* <div className="form-group"> */}
