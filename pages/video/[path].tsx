...
export const getServerSideProps: GetServerSideProps<VideoProps> = async (context) => {
  ...
  const pathArr = [path.substring(0, findex), path.substring(findex + 1, path.length)] //path.substring(0, findex);
        ...
        if (bucketUrl?.url?.toString() != '') {
          ...
          if (errors) {
            ...
            const vidArr = vids.map((video) => {
              const atag = encodeURIComponent(video.partitionKey + '_' + video.sortKey)
  ...
  const handleLike = async () => {
    if (
      ...
      if (
        ...
        JSON.parse(localStorage.getItem('likedTo') ?? '').find((c: any) => c === vidpage?.data?.sortKey)
      ) {
        try {
          ...
          if (videodata) {
            ...
            userliked = userliked.filter((c: any) => c !== vidpage?.data?.sortKey)
        ...
        try {
          ...
          const { data: channeldata, errors } = await client.models.Video.update(
            {
              partitionKey: 'Educational',
              sortKey: vidpage?.data?.sortKey,
          ...
          if (channeldata) {
            ...
            if (localStorage.getItem('likedTo') && localStorage.getItem('likedTo') != null) {
      ...
      if (
        ...
        try {
          ...
          if (vids && vidpage.data) {
            ...
            let newSubbedTo = []
            if (localStorage.getItem('subbedto') && localStorage.getItem('subbedto') != null) newSubbedTo = JSON.parse(localStorage.getItem('subbedto') ?? '[]')
            newSubChannel = {
      ...
      {data && (
        <>
          <div className="video-main">
            <VideoPlayer url={data?.url ?? ''} title={data?.sortKey ?? ''} thumbnail={data?.thumbnail ?? ''} />
            <div className="title">{data?.title ?? data?.sortKey?.split(/[_#]/)[1] ?? ''}</div>
            ...
            <iframe className="youtube-iframe main-iframe" src="https://www.youtube.com/embed/UwsrzCVZAb8?enablejsapi=1"></iframe>

            {data?.debate && (
              <div className="debate-section">
                <h2>Debate</h2>
                {data?.debate?.map((argument, index) => (
                  <div key={index} className={`debate-card ${argument?.self === true && 'right-align'}`}>
                    {typeof window !== undefined && argument?.dp != null && argument?.dp != '' ? (
                      <StorageImage className="card-dp" path={argument?.dp} fallbackSrc="/profile.svg" alt="Profile" />
                    ) : (
                      <img className="card-dp" src="/profile.svg" alt="login" />
                    )}
          ...
          {videoArr && videoArr.length > 0 && (
            <div className="recommended">
              <iframe className="youtube-iframe side-iframe" src="https://www.youtube.com/embed/UwsrzCVZAb8?enablejsapi=1"></iframe>
              <ul className="video-list">{videoArr && videoArr?.length > 0 && videoArr.map((video) => <VideoCard key={video.sortKey} video={video} />)}</ul>
            </div>
          )}
